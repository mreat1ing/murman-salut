...
const CategoriesSpinner: FC = () => {
  const categories = useSelector((state: IStore) => state.storeItemsReducer.categories);
  const categoriesLoading = useSelector((state: IStore) => state.storeItemsReducer.isCategoriesLoading);

  return (
    <div className="categories-spinner">
      {!categoriesLoading ? <ul className="category-spinner__slider">
        {categories && categories.map(item => {if (item.title === 'Все') return (<li key={item.value}>
          <Link to={'/store'}>
          <CategoryCard
            title={item.title}
            value={item.title}
